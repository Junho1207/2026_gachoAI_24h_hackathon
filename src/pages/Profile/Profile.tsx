...
const Profile = () => {
  ...
  const ConnectedIcon = () => (
    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#5C67FF] text-white">
      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path></svg>
    </div>
  );
  
  const VersionIcon = () => (
    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#F0F0F0] text-[#A6A6A6]">
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#FAFAFA] pb-10 font-sans">
      {/* Header */}
      <header className="flex items-center justify-between px-6 py-4">
        <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-blue-100 border-2 border-primary">
          <img src="/src/assets/hero.png" alt="Logo" className="h-full w-full object-cover" onError={(e) => e.currentTarget.style.display='none'} />
        </div>
        <div className="flex items-center gap-3">
          <button className="text-text-primary">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"></path><path d="M13.73 21a2 2 0 0 1-3.46 0"></path></svg>
          </button>
          <button className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#5C67FF] text-white">
      ...
      <main className="flex flex-col px-6 pt-4">
        {error && (
          <div className="mb-4 rounded-xl bg-red-50 p-4 text-sm text-red-500 border border-red-100">
            {error}
          </div>
        )}
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="flex h-24 w-24 items-center justify-center rounded-3xl bg-[#5C67FF] text-white shadow-sm mb-4">
          ...
          <button className="mb-3 rounded-full border border-gray-300 bg-white px-4 py-1 text-sm font-medium text-text-primary shadow-sm">
            {expertTitle}
          </button>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <span className="text-[14px] font-medium text-[#8A8A8A]">ver 1.0</span>
          </div>
          ...
          <div className="flex items-center justify-between rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <span className="text-[14px] font-medium text-[#8A8A8A]">{point.toLocaleString()}</span>
          </div>

          {/* 로그아웃 */}
          <button onClick={handleLogout} className="flex items-center justify-between rounded-2xl border border-gray-200 bg-white p-4 shadow-sm active:bg-gray-50 text-left">
            <div className="flex items-center gap-4">
              <LogoutIcon />
