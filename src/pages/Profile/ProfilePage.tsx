        ...
        <section className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            {user?.expertTitles.map((title) => (
              <div key={title.id} className="flex items-center justify-between rounded-xl border border-border bg-white p-3 px-4">
                ...
                <span className={`rounded-lg px-2 py-0.5 text-[11px] font-bold ${
                  title.level === 'Gold' ? 'bg-yellow-100 text-yellow-700' :
                  title.level === 'Silver' ? 'bg-neutral-100 text-neutral-600' : 'bg-orange-100 text-orange-700'
                }`}>
                  {title.level}
                </span>
              </div>
